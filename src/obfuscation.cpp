...
int randomizeList(int i) { return GetRandInt(i); }
    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat faircoin and make it unusable. To
...
bool CObfuscationPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizefaircoinAmount * COIN - pwalletMain->GetAnonymizedBalance();
...
bool CObfuscationPool::SendRandomPaymentToSelf()
{
    ...
    int64_t nPayment = (nBalance * 0.35) + (GetRandInt(nBalance));
...
void ThreadCheckObfuscationPool()
{
    ...
    RenameThread("faircoin-obfuscation");
