...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizefaircoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizefaircoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizefaircoinAmount = strAnonymizefaircoinAmount.remove(strAnonymizefaircoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizefaircoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizefaircoinAmount * COIN) nMaxToAnonymize = nAnonymizefaircoinAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizefaircoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizefaircoinAmount));
        strAnonymizefaircoinAmount = strAnonymizefaircoinAmount.remove(strAnonymizefaircoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizefaircoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
