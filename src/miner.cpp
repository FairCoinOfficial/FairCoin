...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
    ...
    if(prev->nHeight <= 1) txNew.vout[0].nValue = 23937223 * COIN;
    if(prev->nHeight >= 2) txNew.vout[0].nValue = 10 * COIN;
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("faircoinMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("faircoinMiner started\n");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running faircoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
