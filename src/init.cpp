...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "faircoin.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "faircoind.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 46372, 46374));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, net, faircoin, (obfuscation, swifttx, masternode, mnpayments, mnbudget)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all faircoin specific functionality (Masternodes, Obfuscation, SwiftTX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:46372"));
    ...
    strUsage += HelpMessageOpt("-anonymizefaircoinamount=<n>", strprintf(_("Keep N PIE anonymized (default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 46373, 46375));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The faircoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that faircoin is running in a usable environment with all
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    // Read the optional index flags into the globals before the block index is
    // loaded, so that ConnectBlock/DisconnectBlock and the index-changed checks
    // below see the requested configuration.
        ...
        do {
            try {
                ...
                if (!mapBlockIndex.empty() && mapBlockIndex.count(Params().HashGenesisBlock()) == 0)
                    return InitError(_("Incorrect or no genesis block found. Wrong datadir for network?\nDelete or rename the data directory and restart."));
...
// ********************************************************* Step 8: load wallet
    ...
    nAnonymizefaircoinAmount = GetArg("-anonymizefaircoinamount", 0);
    if (nAnonymizefaircoinAmount > 999999) nAnonymizefaircoinAmount = 999999;
    if (nAnonymizefaircoinAmount < 2) nAnonymizefaircoinAmount = 2;
    ...
    LogPrintf("Anonymize faircoin Amount %d\n", nAnonymizefaircoinAmount);
