...
*Setup instructions for a gitian build of faircoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the faircoin
...
- [Building faircoin](#building-faircoin)
...
Clone the git repositories for faircoin and gitian and then checkout the faircoin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/faircoin-crypto/faircoin.git
cd faircoin
...
Follow the instructions in [doc/release-process.md](release-process.md) in the faircoin repository
...
To build faircoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the faircoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/faircoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/faircoin-crypto/faircoin
...
For example:
```bash
URL=https://github.com/crowning-/faircoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit faircoin=${COMMIT} --url faircoin=${URL} ../faircoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit faircoin=${COMMIT} --url faircoin=${URL} ../faircoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit faircoin=${COMMIT} --url faircoin=${URL} ../faircoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/faircoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/faircoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/faircoin-build.assert
```
...
[faircoin/gitian.sigs](https://github.com/faircoin-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
